...
class Simulation(HttpUser):
    ...
    @task
    def get_home(self):
        response = self.client.post("/trainers/", json={"name": "titouan", "birthdate": "2022-11-07"})
        self.trainer_1_id = response.json()["id"]
        response = self.client.post("/trainers/"+str(self.trainer_1_id)+"/pokemon/", json={"api_id": 120, "custom_name": "titoune"})
